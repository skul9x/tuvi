...
class TuViLogic:
    def __init__(self, solar_day, solar_month, solar_year, hour, gender, name="", viewing_year=None, reading_style="Nghiêm túc"):
        ...
        import datetime
        self.viewing_year = viewing_year if viewing_year else datetime.datetime.now().year
    ...
    def an_sao(self):
        ...
        return {
            "info": {
                "name": self.name,
                "gender": "Nam" if self.gender == 1 else "Nữ",
                "solar_date": f"{self.solar_day}/{self.solar_month}/{self.solar_year}",
                "time": f"{self.hour}h (Giờ {LunarConverter.get_chi_gio(self.hour)})",
